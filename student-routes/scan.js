...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (requestApiKey && requestApiKey !== student.api_key) {
      console.log(
        "⚠️ API key mismatch detected. Checking if UID belongs to the requesting school..."
      );
      ...
      if (sameUidSameSchool.rows.length > 0) {
        console.log(
          "✅ UID found under the requesting API key. Proceeding with this student."
        );
    ...
    if (isSignInTime && !signed_in) {
      ...
      if (nowStr > sign_in_end) {
        existing.punctuality = "late";
      } else {
    ...
    if (isSignOutTime && !signed_out) {
      ...
      // Update punctuality for sign-out
      if (nowStr < sign_out_end) {
        existing.punctuality = "early_leave";
      } else if (!existing.punctuality) {
        existing.punctuality = "on_time"; // keep if not set earlier
      }
    }
