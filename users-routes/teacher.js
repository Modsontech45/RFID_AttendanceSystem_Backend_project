...
router.post('/add', authenticateAdmin, async (req, res) => {
  ...
  try {
    ...
    await sendAcceptanceEmail(email);

    res.status(201).json({ message: 'Teacher added and notified', teacher: result.rows[0] });
  } catch (err) {
...
router.patch('/me', authenticateTeacher, async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Profile updated', teacher: result.rows[0] });
  } catch (err) {
